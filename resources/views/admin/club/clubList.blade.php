...
@section('title',"Club requested list")
@section('css')
            ...
            <tbody>
                @foreach ($users as $key=>$user)
                    <tr>
                        ...
                        <td>{{$user->message}}</td>
                        <td>{{ CustomHelper::prettyDate($user->created_at) }}</td>
                        <td>
{{--                            <a href="{{route('club.edit',$user->id)}}" class="btn btn-primary btn-sm">Edit</a>--}}
                            <form class="delete" action="{{ route('club.destroy',$user->id) }}" method="post">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
